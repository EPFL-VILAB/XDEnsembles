...
""" Default data loading configurations for training, validation, and testing for SIGMA TRAINING. """
def load_train_val_sig(train_tasks, val_tasks=None, fast=False,
    ...
    train_dist_loader = DistTaskDataset(buildings=train_buildings, tasks=train_tasks)
    train_undist_loader = UndistTaskDataset(buildings=train_buildings, tasks=train_tasks)
    print("number of val images:")
    val_dist_loader = DistTaskDataset(buildings=val_buildings, tasks=val_tasks)                # val buildings - distorted data
    val_loader = UndistTaskDataset(buildings=val_buildings, tasks=val_tasks)                   # val buildings - no data aug
    val_ooddist_loader = OODDistTaskDataset(buildings=val_buildings, tasks=val_tasks)          # val buildings - ood distortions
    ...
    train_step = int(len(train_dist_loader) // (400 * batch_size))
    val_step = int(len(val_loader) // (400 * batch_size))
    print("Train step: ", train_step)
    print("Val step: ", val_step)
    if fast: train_step, val_step = 8, 8

    return train_undist_loader, train_dist_loader, val_ooddist_loader, val_dist_loader, val_loader, train_step, val_step


""" Default data loading configurations for training, validation, and testing for CONSISTENCY, BASELINE TRAINING. """
def load_train_val(train_tasks, val_tasks=None, fast=False,
    ...
    val_buildings = val_buildings or (["almena"] if fast else data["val_buildings"])
    print("number of train images:")
    train_loader = UndistTaskDataset(buildings=train_buildings, tasks=train_tasks)
    print("number of val images:")
    val_loader = UndistTaskDataset(buildings=val_buildings, tasks=val_tasks)
...
""" Default data loading configurations for training, validation, and testing for MERGING TRAINING. """
def load_train_val_merging(train_tasks, val_tasks=None, fast=False,
        train_buildings=None, val_buildings=None, split_file="config/split.txt",
        dataset_cls=None, batch_size=32, batch_transforms=cycle,
        subset=None, subset_size=None,
    ):
    ...
    val_buildings = val_buildings or (["almena"] if fast else data["val_buildings"])
    print("number of train images:")
    train_loader = DistTaskDataset(buildings=train_buildings, tasks=train_tasks)
    print("number of val images:")
    val_noaug_loader = UndistTaskDataset(buildings=val_buildings, tasks=val_tasks)                # val buildings - no data aug
    val_loader = DistTaskDataset(buildings=val_buildings, tasks=val_tasks)                 # val buildings - same data aug as train set 
    ...
    return train_loader, val_loader, val_noaug_loader, train_step, val_step
...
def load_ood(tasks=[tasks.rgb], ood_path=OOD_DIR, sample=28):
...
class UndistTaskDataset(TaskDataset):
...
class DistTaskDataset(TaskDataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(200):
            try:
                ...
                for task in self.tasks:
                    distortion_ind = random.randint(0,1) if task.name == 'rgb' else None
                    ...
                    file_name = self.convert_path(self.idx_files[idx], task)
                    if len(file_name) == 0: raise Exception("unable to convert file")
                    image = task.file_loader(file_name, resize=self.resize, seed=seed, crop=crop, jitter=False, blur_radius=blur_noise_jpeg[0], noise=blur_noise_jpeg[1], jpeg=None)
                    res.append(image)
...
class OODDistTaskDataset(TaskDataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(200):
            try:
                ...
                distortion_list = ['zoom_blur','defocus_blur','pixelate','shot_noise','impulse_noise','jpeg_compression']
                severity = random.randint(1,5)
                selected_distortion = random.choice(distortion_list)

                for task in self.tasks:

                    file_name = self.convert_path(self.idx_files[idx], task)
                    if len(file_name) == 0: raise Exception("unable to convert file")
                    image = task.file_loader(file_name, resize=self.resize, seed=seed, crop=512, val_distortion_name=selected_distortion, val_severity=severity)
                    res.append(image)
